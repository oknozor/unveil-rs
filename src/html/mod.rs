...
impl Preprocessor {
        ...
        html(lang="EN") {
            ...
            body {
               ...
               div(onclick="next_slide_left()", class="arrow-left") {
                    ...
                    data-icon="chevron-left", class="svg-inline--fa fa-chevron-right fa-w-10",
                    role="img", xmlns="http://www.w3.org/2000/svg", viewBox="0 0 320 512")
                    {
                        path(
                            fill="currentColor",
    ...
    fn insert_playpen_button(&mut self) {
        // Who needs a parser anyway ?!
        let button = html! {
            div(onclick="fetch_with_timeout()", class="btn-playpen") {
                    svg(
        ...
        let replace = format!("{}{}", r#"<code class="language-rust">"#, button.to_string());

        self.html = self.html.replace(r#"<code class="language-rust">"#, &replace);
    }
